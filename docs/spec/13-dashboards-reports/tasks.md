# Tasks — Dashboard & laporan

| ID | Task | Prioritas | Status | Catatan |
|---|---|---|---|---|
| T01 | `DashboardController` invoke per role | P0 | done | |
| T02 | Pages Dashboard/{Admin,Guru,Siswa,WaliKelas,WaliMurid,Generic} | P0 | done | |
| T03 | `WaliMuridController` data anak | P0 | done | |
| T04 | `TeacherReportController` + Reports/Teacher.vue | P0 | done | |
| T05 | KPI/hub guru di Dashboard/Guru | P1 | done | |
| T06 | Smoke di AksaraTest / AdminOversightTest | P1 | done | |
| T07 | Perkaya laporan wali kelas | P2 | todo | debt akses |
